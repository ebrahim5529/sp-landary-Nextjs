...
import { generateInvoiceNumber, generateOrderNumber, calculateTax, calculateTotal } from "@/utils/invoice";
...
export default function Cashier() {
  ...
  const handleItemServicesChange = (index: number, services: { service_id: string; price: number }[]) => {
      ...
      <div className="h-[calc(100vh-8rem)] flex flex-col" dir="rtl">
        <Tabs defaultValue="new" className="flex-1 flex flex-col">
          ...
          <TabsContent value="recent" className="flex-1 overflow-y-auto">
            <InvoiceList
              onViewInvoice={(id) => navigate(`/dashboard/invoices?invoice=${id}`)}
              onPrintInvoice={(id) => navigate(`/dashboard/invoices?invoice=${id}&print=true`)}
            />
          </TabsContent>
        </Tabs>
      </div>
